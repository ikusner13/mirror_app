...
function Auth(config, debug = false) {
  ...
  if (config.keyFilePath === undefined) {
    throw new Error(
      'Missing "keyFilePath" from config (This should be where your Credential file is)'
    )
  }
  if (config.savedTokensPath === undefined) {
    throw new Error(
      'Missing "savedTokensPath" from config (this should be where your OAuth2 access tokens will be saved)'
    )
    return
  }
...
class GPhotos {
  ...
  generateToken(success = () => {}, fail = () => {}) {
    this.onAuthReady((client) => {
      const isTokenFileExist = () => {
        var fp = path.resolve(
          __dirname,
          this.options.authOption.savedTokensPath
        )
  ...
  getAlbumType(type = 'albums') {
    if (type !== 'albums' && type !== 'sharedAlbums')
      throw new Error('Invalid parameter for .getAlbumType()', type)
    return new Promise((resolve) => {
      this.onAuthReady((client) => {
      ...
      this.onAuthReady((client) => {
        ...
        const getImage = async (pageSize = 50, pageToken = '') => {
          ...
          try {
            ...
            if (
              response.data.hasOwnProperty('mediaItems') &&
              Array.isArray(response.data.mediaItems)
            ) {
